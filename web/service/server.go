...
func (s *ServerService) GetDb() ([]byte, error) {
	// Для MySQL возвращаем информацию о подключении к базе данных
	dbInfo := map[string]interface{}{
		...
		"message":       "Use mysqldump for database backup/restore",
	}

	jsonData, err := json.Marshal(dbInfo)
	if err != nil {
		return nil, err
	}
...
func (s *ServerService) ImportDB(file multipart.File) error {
	// Для MySQL импорт базы данных не поддерживается через файл
	// Вместо этого нужно использовать mysqldump для экспорта/импорта
	return common.NewError("Import DB is not supported for MySQL. Use mysqldump for database backup/restore.")
}
