...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://raw.githubusercontent.com/cryptonceo/3x-ui/v$tag_version/install.sh") v$tag_version"
...
uninstall() {
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/cryptonceo/3x-ui/master/install.sh)${plain}"
    echo ""
...
update_shell() {
    wget -O /usr/bin/x-ui -N https://github.com/cryptonceo/3x-ui/raw/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
